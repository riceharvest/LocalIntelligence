...
class HarnessSelfTest {
    ...
    fun `the parser reads a respond action`() {
        ...
        assertEquals("hello", (parsed as ActionParseResult.Parsed).action.let {
            (it as dev.localintelligence.core.agent.AgentAction.Respond).text
        })
    }
    ...
    fun `the parser reads a call action with arguments`() {
        ...
        assertTrue(action is dev.localintelligence.core.agent.AgentAction.CallTool)
        assertEquals("hi", (action as dev.localintelligence.core.agent.AgentAction.CallTool).arguments.flatten()["text"])
    }
    ...
    private fun ctx() = dev.localintelligence.core.tool.ToolContext()

    private fun request() = dev.localintelligence.core.model.GenerationRequest(
        messages = listOf(
            dev.localintelligence.core.model.ChatMessage.System("SYSTEM you are a phone agent"),
            dev.localintelligence.core.model.ChatMessage.User("what's my battery"),
        )
    )
