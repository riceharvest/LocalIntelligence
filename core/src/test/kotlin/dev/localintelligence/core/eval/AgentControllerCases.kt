...
import dev.localintelligence.core.agent.ActionParseResult
...
internal class ReferenceContextBuilder(
    ...
    override fun build(
        ...
        tools: List<dev.localintelligence.core.tool.ToolDefinition>,
    ): List<ChatMessage> = buildList {
        add(ChatMessage.System(dev.localintelligence.core.context.SystemPrompts.forTools(tools)))
...
class ReferenceAgentLoop(
    ...
    override suspend fun run(task: EvalTask): CompletedRun {
        ...
        val memory: MemoryStore = dev.localintelligence.core.agent.InMemoryMemoryStore()
            ...
            when (val parsed = parser.parse(generation.text, visibleNames.toSet())) {
                ...
                is ActionParseResult.Parsed -> when (val action = parsed.action) {
                    ...
                    is AgentAction.CallTool -> {
                        ...
                        val result = try {
                            tool.execute(action.arguments, dev.localintelligence.core.tool.ToolContext())
                        } catch (e: Exception) {
                            ToolResult(
                                success = false,
                                observation = "${tool.definition.name} failed: ${e.message ?: e::class.simpleName}",
                                error = dev.localintelligence.core.tool.ToolError.Internal(e.message ?: "tool threw"),
                            )
                        }
...
private fun buildOne(name: String, task: EvalTask): ScriptedTool {
    ...
    return when {
        ...
        denied -> ScriptedTool(
            ...
            results = listOf(
                dev.localintelligence.core.tool.ToolResult(
                    success = false,
                    observation = "Permission denied: $name needs ${spec.permission ?: "a permission"}. " +
                        "Grant it in Settings to let me do that.",
                    error = dev.localintelligence.core.tool.ToolError.PermissionDenied(spec.permission ?: "unknown"),
                )
            ),
        )
        empty -> ScriptedTool(
            definition = base.definition,
            results = listOf(
                dev.localintelligence.core.tool.ToolResult(success = true, observation = "No results for \"$name\".")
            ),
        )
        huge -> ScriptedTool(
            definition = base.definition,
        ...
        override != null -> ScriptedTool(
            definition = base.definition,
            results = listOf(dev.localintelligence.core.tool.ToolResult(success = true, observation = override)),
        )
        else -> base
    }
}
...
//
//  1. `dev.localintelligence.core.agent.AgentController` exists, with the exact
...
//  2. `dev.localintelligence.core.agent.AgentConfig`, `AgentResult`, `StepTrace` exist as
//     pinned, including `StepTrace.Kind` with all five kinds.
//  3. `dev.localintelligence.core.agent.Session` exists (or the compact() signature was
//     changed to take (history, summary, model) as the contract allows).
//  4. `dev.localintelligence.core.agent.ToolCallValidatorGate` + `ValidationOutcome`
//     exist as pinned, so `MockToolRegistry.gate()` can be adapted by mapping
//     GateOutcome -> ValidationOutcome.
//  5. `dev.localintelligence.core.context.DefaultContextBuilder` exists, or the harness
