 ...
 * Android and a memory hazard in a per-step budget check. [candidate]
 * [clips] the proposed content to a stated ceiling before pricing it, so the
 * check itself stays cheap and bounded regardless of what a tool returns.
 */
class StepEnforcer(
    ...
    fun evaluate(
        ...
        val reply = counter.count(proposed.replyCandidate)
        val arguments = counter.count(proposed.toolArguments)
        ...
        val observation = counter.count(proposed.observationCandidate)
        val summary = counter.count(proposed.summaryCandidate)
    ...
    companion object {
         ...
         *
         * Applied by [ProposedStep] construction, so no caller can skip it by
         * forgetting.
         */
...
data class ProposedStep(
    ...
    init {
        require(replyCandidate.length <= MAX) {
            "replyCandidate of ${replyCandidate.length} chars exceeds the " +
                "$MAX per-step bound; truncate at the source"
        }
        require(toolArguments.length <= MAX) {
            "toolArguments of ${toolArguments.length} chars exceeds the $MAX bound"
        }
        require(observationCandidate.length <= MAX) {
            "observationCandidate of ${observationCandidate.length} chars exceeds " +
                "the $MAX bound; architecture section 7 caps observations at 2048"
        }
        require(summaryCandidate.length <= MAX) {
            "summaryCandidate of ${summaryCandidate.length} chars exceeds the $MAX bound"
        }
    }

    private companion object {
        const val MAX = StepEnforcer.MAX_CANDIDATE_CHARS
    }
