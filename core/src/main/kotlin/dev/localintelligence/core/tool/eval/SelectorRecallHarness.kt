...
class SelectorRecallHarness(
    ...
    fun widthSafetyReport(k: Int): String {
        ...
        // NOT `config.prefillCostCapTokens`. That is the prefill COST cap; the
        // prompt ceiling is the smaller of the model's own window and that cap,
        // and this harness has no model, so `ContextCeiling.workingLimit(null)`
        // is the value that means the same thing it used to. The old
        // `AgentConfig.workingTokenLimit` (6000) was renamed precisely because
        // it read as a ceiling and was not one.
        val workingLimit = ContextCeiling.workingLimit(null)
        val promptCeiling = workingLimit - CONTEXT_BUDGET_OUTPUT_RESERVE
        ...
        return buildString {
            ...
            appendLine("    prompt ceiling (workingLimit $workingLimit - $CONTEXT_BUDGET_OUTPUT_RESERVE reserve): $promptCeiling")
