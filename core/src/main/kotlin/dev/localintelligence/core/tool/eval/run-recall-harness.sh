#!/usr/bin/env bash
# Runs the tool-selection recall harness on a plain JVM. No JUnit, no Gradle
# task, no device, no emulator.
#
...
# and the repository root is nine levels above `eval/`. Counting `..` segments
# is the kind of thing that breaks silently the day a package is renamed, so
# walk up to the first directory that actually looks like the repository root
# and fail loudly if there isn't one.
...
exec "$JAVA_HOME/bin/java" -cp "$CP" \
  dev.localintelligence.core.tool.eval.SelectorRecallHarnessKt "$@"
