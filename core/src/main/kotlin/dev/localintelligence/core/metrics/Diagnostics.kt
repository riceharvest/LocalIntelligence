...
object RunPerformanceCheck {
    ...
    fun unknown(): DiagnosticCheck = DiagnosticCheck(
        ...
        verified = "Looked for a measured THROUGHPUT figure in this project: a decode " +
            "rate, or a time to first token, taken on physical hardware.",
        finding = "There are none. Zero measured throughput figures exist for a real " +
            ...
            "CPU. It is not a phone number and it is not shown as one. Note that this " +
            "card is about SPEED only: the process's actual memory use is measured on " +
            "this device and is on the card above.",
        cannotTell = "How fast this phone generates. Decode rate and time to first " +
            "token are unknown, and no amount of reading this app will produce them — " +
            "nothing in the codebase samples either, because the loop streams whole " +
            "generations and a first-token latency cannot be recovered from a run's " +
            "total prefill time. What this screen CAN show you is what one run on this " +
            "phone did: the token counts and per-phase durations the loop measured " +
            "during it, which is a real measurement of that run and not a specification " +
            "of the hardware.",
        findings = listOf(
            Finding.unknown(
                "Decode throughput",
